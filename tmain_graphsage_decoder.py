...
def build_graphsage_model(args):
    ...
    dataset_loader, node_num, feature_dim = GraphLoader.load_graph(graph_attribute_list,
                graph_adj_list, graph_inv_deg_list, graph_label_list, args)
...
def test_graphsage_model(dataset, model, args):
    for batch_idx, data in enumerate(dataset):
        adj = Variable(data['adj'].float(), requires_grad = False).cuda()
        inv_deg = Variable(data['inv_deg'].float(), requires_grad = False).cuda()
        x = Variable(data['feature'].float(), requires_grad = False).cuda()
...
def test_graphsage(args):
    # BUILD [GraphSAGE, DECODER] MODEL
    model = build_graphsage_model(args)
    model.load_state_dict(torch.load('./datainfo2/result/epoch_100_2.2/best_train_model.pth'))
    ...
    for index in range(0, dl_input_num, batch_size):
        ...
        dataset_loader, node_num, feature_dim = GraphLoader.load_graph(graph_attribute_list,
                    graph_adj_list, graph_inv_deg_list, graph_label_list, prog_args)
        # ACTIVATE LEARNING RATE SCHEDULE
        model, batch_loss, batch_ypred = test_graphsage_model(dataset_loader, model, args)
        batch_loss = batch_loss.item()
        print('BATCH LOSS: ', batch_loss)
        batch_loss_list.append(batch_loss)
        # PRESERVE PREDICTION OF BATCH TRAINING DATA
...
if __name__ == "__main__":
    ...
    train_graphsage(prog_args)
    # test_graphsage(prog_args)
