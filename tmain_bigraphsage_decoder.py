...
from load.read_bigraph import read_train_batch, read_test_batch
from load.bigraph_sampler import BiGraphLoader
from enc_dec.bigraphsage_decoder import BiGraphSAGEDecoder
...
def build_bigraphsage_model(args):
    ...
    graph_attribute_list, graph_adj_list, graph_up_inv_deg_list, graph_down_inv_deg_list, graph_label_list = read_train_batch(0, 1, args)
    dataset_loader, node_num, feature_dim = BiGraphLoader.load_graph(graph_attribute_list,
                graph_adj_list, graph_up_inv_deg_list, graph_down_inv_deg_list, graph_label_list, args)
    model = BiGraphSAGEDecoder(args.add_self, input_dim, hidden_dim, embedding_dim, decoder_dim, node_num, num_layer, dropout)
    return model

def train_bigraphsage_model(dataset, model, args, learning_rate):
    optimizer = torch.optim.Adam(filter(lambda p : p.requires_grad, model.parameters()), lr = learning_rate)
    # optimizer = torch.optim.SGD(model.parameters(), lr = args.lr, momentum = 0.9)
    for batch_idx, data in enumerate(dataset):
        optimizer.zero_grad()
        adj = Variable(data['adj'].float(), requires_grad = False).cuda()
        up_inv_deg = Variable(data['up_inv_deg'].float(), requires_grad = False).cuda()
        down_inv_deg = Variable(data['down_inv_deg'].float(), requires_grad = False).cuda()
        ...
        ypred = model(x, adj, up_inv_deg, down_inv_deg)
...
def train_bigraphsage(args):
    # BUILD [BiGraphSAGE, DECODER] MODEL
    model = build_bigraphsage_model(args)
    if args.model == 'load':
        model.load_state_dict(torch.load('./datainfo2/result/epoch_75/best_train_model.pth'))
    ...
    for i in range(1, epoch_num + 1):
        ...
        for index in range(0, dl_input_num, batch_size):
            ...
            graph_attribute_list, graph_adj_list, graph_up_inv_deg_list, graph_down_inv_deg_list, graph_label_list = read_train_batch(index, upper_index, args)
            dataset_loader, node_num, feature_dim = BiGraphLoader.load_graph(graph_attribute_list,
                        graph_adj_list, graph_up_inv_deg_list, graph_down_inv_deg_list, graph_label_list, prog_args)
            # ACTIVATE LEARNING RATE SCHEDULE
            iteration_num += 1
            learning_rate = learning_rate_schedule(args, dl_input_num, iteration_num, e1, e2, e3)
            # learning_rate = 0.005
            model, batch_loss, batch_ypred = train_bigraphsage_model(dataset_loader, model, args, learning_rate)
...
def test_bigraphsage_model(dataset, model, args):
    for batch_idx, data in enumerate(dataset):
        adj = Variable(data['adj'].float(), requires_grad = False).cuda()
        up_inv_deg = Variable(data['up_inv_deg'].float(), requires_grad = False).cuda()
        down_inv_deg = Variable(data['down_inv_deg'].float(), requires_grad = False).cuda()
...
def test_bigraphsage(args):
    ...
    for index in range(0, dl_input_num, batch_size):
        if (index + batch_size) < dl_input_num:
            ...
            upper_index = dl_input_num
        graph_attribute_list, graph_adj_list, graph_up_inv_deg_list, graph_down_inv_deg_list, graph_label_list = read_test_batch(index, upper_index, args)
        dataset_loader, node_num, feature_dim = BiGraphLoader.load_graph(graph_attribute_list,
                    graph_adj_list, graph_up_inv_deg_list, graph_down_inv_deg_list, graph_label_list, prog_args)
        # ACTIVATE LEARNING RATE SCHEDULE
        model, batch_loss, batch_ypred = test_bigraphsage_model(dataset_loader, model, args)
        batch_loss = batch_loss.item()
        print('BATCH LOSS: ', batch_loss)
        batch_loss_list.append(batch_loss)
        # PRESERVE PREDICTION OF BATCH TRAINING DATA
