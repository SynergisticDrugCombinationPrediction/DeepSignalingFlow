...
from enc_dec.gat_decoder import GATDecoder
...
def build_gat_model(args):
    ...
    model = GATDecoder(args.add_self, input_dim, hidden_dim, embedding_dim, decoder_dim, node_num, num_layer)
    return model

def train_gat_model(dataset, model, args, learning_rate):
...
def train_gat(args):
    # BUILD [GAT, DECODER] MODEL
    model = build_gat_model(args)
    if args.model == 'load':
    ...
    for i in range(1, epoch_num + 1):
        ...
        for index in range(0, dl_input_num, batch_size):
            ...
            iteration_num += 1
            learning_rate = learning_rate_schedule(args, dl_input_num, iteration_num, e1, e2)
            model, batch_loss, batch_ypred = train_gat_model(dataset_loader, model, args, learning_rate)
...
def test_gat_model(dataset, model, args):
    for batch_idx, data in enumerate(dataset):
        adj = Variable(data['adj'].float(), requires_grad = False)
        inv_deg = Variable(data['inv_deg'].float(), requires_grad = False)
        x = Variable(data['feature'].float(), requires_grad = False)
...
def test_gat(args):
    ...
    batch_size = args.batch_size
    # CLEAN RESULT PREVIOUS EPOCH_I_PRED FILES
    path = '.' + dir_opt + '/result/epoch_50_1'
    # RUN TEST MODEL
    model.eval()
    ...
    for index in range(0, dl_input_num, batch_size):
        ...
        dataset_loader, node_num, feature_dim = GraphLoader.load_graph(graph_attribute_list,
                    graph_adj_list, graph_inv_deg_list, graph_label_list, prog_args)
        # ACTIVATE LEARNING RATE SCHEDULE
        model, batch_loss, batch_ypred = test_gat_model(dataset_loader, model, args)
        batch_loss = batch_loss.item()
        print('BATCH LOSS: ', batch_loss)
        batch_loss_list.append(batch_loss)
        # PRESERVE PREDICTION OF BATCH TRAINING DATA
    ...
    tmp_test_input_df = pd.read_csv('.' + dir_opt + '/filtered_data/TestInput.txt', delimiter = ',')
    final_row, final_col = tmp_test_input_df.shape
    all_ypred_lists = list(all_ypred)
    all_ypred_list = [item for elem in all_ypred_lists for item in elem]
    tmp_test_input_df.insert(final_col, 'Pred Score', all_ypred_list, True)
