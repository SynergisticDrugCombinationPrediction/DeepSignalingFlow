...
from enc_dec.gcn_decoder import GcnDecoderGraph
...
def learning_rate_schedule(args, dl_input_num, iteration_num, e1, e2):
    ...
    if iteration_num <= (e1 *epoch_iteration):
        ...
        learning_rate = 0.01 - (iteration_num - e1 * epoch_iteration) * l2
    else:
        learning_rate = l3
    print('-------LEARNING RATE ' + str(learning_rate) + '-------' )
    return learning_rate

def build_gcn_model(args):
    ...
    model = GcnDecoderGraph(args.add_self, input_dim, hidden_dim, embedding_dim, decoder_dim, node_num, num_layer)
    return model
...
def train_gcn(args):
    # BUILD [GCN, DECODER] MODEL
    model = build_gcn_model(args)
    if args.model == 'load':
...
def test_gcn_model(dataset, model, args):
    for batch_idx, data in enumerate(dataset):
        adj = Variable(data['adj'].float(), requires_grad = False)
        inv_deg = Variable(data['inv_deg'].float(), requires_grad = False)
        x = Variable(data['feature'].float(), requires_grad = False)
...
def test_gcn(args):
    ...
    for index in range(0, dl_input_num, batch_size):
        ...
        dataset_loader, node_num, feature_dim = GraphLoader.load_graph(graph_attribute_list,
                    graph_adj_list, graph_inv_deg_list, graph_label_list, prog_args)
        # ACTIVATE LEARNING RATE SCHEDULE
        model, batch_loss, batch_ypred = test_gcn_model(dataset_loader, model, args)
        batch_loss = batch_loss.item()
        print('BATCH LOSS: ', batch_loss)
        batch_loss_list.append(batch_loss)
        # PRESERVE PREDICTION OF BATCH TRAINING DATA
